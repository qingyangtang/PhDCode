...
from sptpol_software.analysis.powspec import make_masks, covariance_utils
...
def makeBorderApodization(_map, apod_type='cos', weight_threshold=0.3, radius_arcmin=20.,
    ...
    if given_weight_map:
        weight = _map
        t_map = struct({'reso_arcmin':given_reso_arcmin})
    else:
        if isinstance(_map, sky.Map):
            t_map = _map
        else:
            t_map = _map.getTOnly()
        weight = t_map.weight
    ...
    if smooth_weights_arcmin > 0.:
        weight = ndimage.gaussian_filter(weight, smooth_weights_arcmin / t_map.reso_arcmin / (2*np.sqrt(2*np.log(2)))) # Not certain we need the normalization.

    # Get the weight array, normalized by either the maximum or the median (nonzero) weight.
    if threshold_median:
        if verbose: print "Thresholding based on the median weight."
        weight = weight / math.cMedian(weight[weight!=0.], cast_to_float=True)
    else:
        if verbose: print "Thresholding based on the maximum weight."
        weight = weight / weight.max() # Get weight, normalized to a maximum of 1.

    # Create the mask, and start by masking pixels with too low a weight.
    mask = np.ones(weight.shape, dtype=np.float)
    mask[weight<weight_threshold] = 0.
    ...
    if zero_border_arcmin > 0:
        if verbose: print "Padding the border with zeros to a depth of %f arcminutes (%d pixels)." % (zero_border_arcmin, int(zero_border_arcmin / t_map.reso_arcmin))
        distance_transform = ndimage.distance_transform_edt(mask, sampling=t_map.reso_arcmin)
    ...
    if apod_type.lower().startswith('gaus'):
        ...
        mask = ndimage.gaussian_filter(mask, radius_arcmin / t_map.reso_arcmin / (2*np.sqrt(2*np.log(2)))) # Not certain we need the normalization.
    elif apod_type.lower().startswith('cos') and (radius_arcmin > 0):
        # Use a cosine function
        npix_cos = int(radius_arcmin / t_map.reso_arcmin)
...
##########################################################################################
def makeApodizedPointSourceMask(Map, source_mask, flux_thresh=None,apod_type='cos', radius_arcmin=20.,
                                zero_border_arcmin=0., verbose=True,
                                center=np.asarray([352.50226, -55.000390]),fixed_rad_deg=None,
                                apod_threshold=1e-8):
    """
   ...
   INPUTS
    ...
    try:
        ...
        try:
            ...
            mask = make_masks.makePtSrcMask(point_source_config, t_map, center=center, flux_thresh=flux_thresh, fixed_rad_deg=fixed_rad_deg)
        except:
