...
def makePtSrcMask_herschel(point_source_config, field_map, reso_arcmin,dir_out = '/data/tangq/SPIRE_maps/', debug = False,
                  center = np.array([352.51485, -54.99948]), fixed_rad_deg=None,flux_thresh=None, proj=0):
    '''INPUTS
    ...
    map_ras, map_dcs = sky.pix2Ang((np.repeat(np.arange(tmap.shape[0]),tmap.shape[1]),
                                     np.tile(np.arange(tmap.shape[1]),tmap.shape[0])),center, reso_arcmin,npixels,proj=proj)
    ...
    cat_ras = catalog[:,0][use_mask]
    cat_dcs = catalog[:,1][use_mask]
    cat_flux = catalog[:,2][use_mask]
    cat_dflux = catalog[:,3][use_mask]
    sn = cat_flux/cat_dflux
    cat_mds = np.zeros((len(cat_flux)))
    cat_mds[np.where(sn > 50)[0]] = 6*0.605/60.
    cat_mds[np.where(sn <= 50)[0]] = 4*0.605/60.
    ...
    np.save(dir_out + 'hershel_ptsrc_mask.npy', mask)
...
def makePtSrcMask(point_source_config, field_map, reso_arcmin,dir_out = None, debug = False,
    ...
    if dir_out != None:
        fileObj = open(dir_out + '/ptsrc_mask.pkl', 'w')
        pickle.dump(mask, fileObj)
        fileObj.close()
