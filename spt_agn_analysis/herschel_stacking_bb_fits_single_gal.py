...
true_mask = np.zeros((zp_mask.shape))
true_mask[zp_mask>0.9999999]=1
...
for i in range(len(sm_bins)-1):
	...
	for b, bin_width in enumerate([0.1, 0.2, 0.5]):
		...
		num_obj_zbins = np.zeros((len(z_bins[:-1])))
		n_realizations = 1000
		sn_arr_mean = np.zeros((len(z_bins[:-1]), 3))
		sn_arr_med = np.zeros((len(z_bins[:-1]), 3))
		...
		for x in range(3):
			...
			for z_i in range(len(z_bins)-1):
				...
				galaxy_pixels = cmap[ypix_i,xpix_i]
				rand_pix = np.random.choice(cmap[true_mask], len(ypix_i))

				noise_mad = t.mad(rand_pix)
				sn_arr_med[z_i,x] = np.median(cmap[ypix_i,xpix_i])/noise_mad
				sn_arr_mean[z_i,x] = np.mean(cmap[ypix_i,xpix_i])/noise_mad
		for j in range(3):
			axarr1[0, j].plot(z_bins[:-1], sn_arr_mean[:,j], color=bw_colors[b], label=r'$\Delta$z='+str(bin_width))
			axarr1[0, j].plot(z_bins[:-1], sn_arr_med[:,j], ls = 'dotted', color=bw_colors[b])
			axarr1[1, j].plot(z_bins[:-1], sn_arr_mean[:,j]/num_obj_zbins, color=bw_colors[b], label=r'$\Delta$z='+str(bin_width))
			axarr1[1, j].plot(z_bins[:-1], sn_arr_med[:,j]/num_obj_zbins, ls = 'dotted', color=bw_colors[b])
			axarr1[2, j].plot(z_bins[:-1], 5./(sn_arr_mean[:,j]/num_obj_zbins), color=bw_colors[b], label=r'$\Delta$z='+str(bin_width))
			axarr1[2, j].plot(z_bins[:-1], 5./(sn_arr_med[:,j]/num_obj_zbins), ls = 'dotted', color=bw_colors[b])
			axarr1[3, j].plot(z_bins[:-1], num_obj_zbins, color=bw_colors[b], label=r'$\Delta$z='+str(bin_width))
	for l in range(3):
	...
	axarr1[3, 0].set_xlabel('z (250um data)')
	axarr1[3, 1].set_xlabel('z (350um data)')
	axarr1[3, 2].set_xlabel('z (500um data)')
	...
	fig.suptitle('Mean (solid) or median (dotted) S/N per obj for varying redshift bins at SM=' +str((sm_bins[i]+sm_bins[i+1])/2.))
	plt.savefig('/data/tangq/blackbody_plots/'+today+'Herschel_SN_mean_z_bins_SM'+str(i)+'1.png')
	plt.close()
